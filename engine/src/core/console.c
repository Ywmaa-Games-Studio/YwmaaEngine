...
b8 console_init(u64* memory_requirement, void* memory, void* config) {
    *memory_requirement = sizeof(CONSOLE_STATE) + (sizeof(CONSOLE_CONSUMER) * MAX_CONSUMER_COUNT);
    ...
    yzero_memory(memory, *memory_requirement);
    state_ptr = memory;
...
void console_register_consumer(void* inst, PFN_console_consumer_write callback, u8* out_consumer_id) {
    if (state_ptr) {
        YASSERT_MSG(state_ptr->consumer_count + 1 < MAX_CONSUMER_COUNT, "Max console consumers reached.");
...
void console_update_consumer(u8 consumer_id, void* inst, PFN_console_consumer_write callback) {
    if (state_ptr) {
        YASSERT_MSG(consumer_id < state_ptr->consumer_count, "Consumer id is invalid.");
...
b8 console_register_command(const char* command, u8 arg_count, PFN_console_command func) {
    YASSERT_MSG(state_ptr && command, "console_register_command requires state and valid command");
    ...
    for (u32 i = 0; i < command_count; ++i) {
        if (strings_equali(state_ptr->registered_commands[i].name, command)) {
            PRINT_ERROR("Command already registered: %s", command);
            return false;
        }
    }

    CONSOLE_COMMAND new_command = {0};
...
b8 console_unregister_command(const char* command) {
    YASSERT_MSG(state_ptr && command, "console_update_command requires state and valid command");
...
b8 console_execute_command(const char* command) {
    ...
    for (u32 i = 0; i < command_count; ++i) {
        ...
        if (strings_equali(cmd->name, parts[0])) {
            ...
            if (state_ptr->registered_commands[i].arg_count != arg_count) {
                PRINT_ERROR("The console command '%s' requires %u arguments but %u were provided.", cmd->name, cmd->arg_count, arg_count);
                has_error = true;
            } else {
                ...
                if (context.argument_count > 0) {
                    context.arguments = yallocate_aligned(sizeof(CONSOLE_COMMAND_ARGUMENT) * cmd->arg_count, 8, MEMORY_TAG_ARRAY);
    ...
    if (!command_found) {
        PRINT_ERROR("The command '%s' does not exist.", parts[0]);
        has_error = true;
    }
