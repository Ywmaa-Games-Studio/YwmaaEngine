...
void identifier_release_id(u32 id) {
    if (!owners) {
        PRINT_ERROR("identifier_release_id called before initialization. identifier_aquire_new_id should have been called first. Nothing was done.");
        return;
    }

    u64 length = darray_length(owners);
    if (id > length) {
        PRINT_ERROR("identifier_release_id: id '%u' out of range (max=%llu). Nothing was done.", id, length);
        return;
    }
