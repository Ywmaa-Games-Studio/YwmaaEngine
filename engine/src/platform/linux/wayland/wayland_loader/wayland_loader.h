...
#define VIDEO_DRIVER_WAYLAND_DYNAMIC "libwayland-client.so"
#define VIDEO_DRIVER_WAYLAND_DYNAMIC_CURSOR "libwayland-cursor.so"

#ifndef waylanddyn_h_
#define waylanddyn_h_
...
#define WAYLAND_CHECK_VERSION(x, y, z)                        \
...
#define LIBDECOR_CHECK_VERSION(x, y, z)                                 \
    (LIBDECOR_VERSION_MAJOR > x ||                                      \
     (LIBDECOR_VERSION_MAJOR == x && LIBDECOR_VERSION_MINOR > y) || \
     (LIBDECOR_VERSION_MAJOR == x && LIBDECOR_VERSION_MINOR == y && LIBDECOR_VERSION_PATCH >= z))
#endif
...
extern int WAYLAND_LoadSymbols(void);
extern void WAYLAND_UnloadSymbols(void);

#define WAYLAND_MODULE(modname) extern int WAYLAND_HAVE_##modname;
#define WAYLAND_SYM(rc, fn, params)        \
    typedef rc(*DYNWAYLANDFN_##fn) params; \
    extern DYNWAYLANDFN_##fn WAYLAND_##fn;
#define WAYLAND_SYM_OPT(rc, fn, params)    \
    typedef rc(*DYNWAYLANDFN_##fn) params; \
    extern DYNWAYLANDFN_##fn WAYLAND_##fn;
#define WAYLAND_INTERFACE(iface) extern const struct wl_interface *WAYLAND_##iface;
...
#error Do not include wayland-client ahead of wayland_loader.h in dynamic loading mode
#endif
...
#else // VIDEO_DRIVER_WAYLAND_DYNAMIC
