...
- (void)keyDown:(NSEvent *)event {
    E_KEYS key = translate_keycode((u32)[event keyCode]);
...
b8 platform_system_startup(u64* memory_requirement, void* state, void* config) {
    ...
    *memory_requirement = sizeof(PLATFORM_STATE);
    ...
    if (!state_ptr->app_delegate) {
        PRINT_ERROR("Failed to create application delegate")
        return false;
    }
    ...
    if (!state_ptr->wnd_delegate) {
        PRINT_ERROR("Failed to create window delegate")
        return false;
    }
    ...
    if (!state_ptr->layer) {
        PRINT_ERROR("Failed to create layer for view");
    }
    ...
    state_ptr->layer.contentsScale = state_ptr->view.window.backingScaleFactor;
    PRINT_DEBUG("contentScale: %f", state_ptr->layer.contentsScale);
...
b8 platform_create_webgpu_surface(WEBGPU_CONTEXT *context) {
    ...
    surface_metal_layer.layer = state_ptr->layer;
