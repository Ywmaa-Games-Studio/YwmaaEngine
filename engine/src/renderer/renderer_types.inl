...
typedef enum E_RENDER_TARGET_ATTACHMENT_TYPE {
    RENDER_TARGET_ATTACHMENT_TYPE_COLOR = 0x1,
    RENDER_TARGET_ATTACHMENT_TYPE_DEPTH = 0x2,
    RENDER_TARGET_ATTACHMENT_TYPE_STENCIL = 0x4
} E_RENDER_TARGET_ATTACHMENT_TYPE;

typedef enum E_RENDER_TARGET_ATTACHMENT_SOURCE {
    RENDER_TARGET_ATTACHMENT_SOURCE_DEFAULT = 0x1,
    RENDER_TARGET_ATTACHMENT_SOURCE_VIEW = 0x2
} E_RENDER_TARGET_ATTACHMENT_SOURCE;

typedef enum E_RENDER_TARGET_ATTACHMENT_LOAD_OPERATION {
    RENDER_TARGET_ATTACHMENT_LOAD_OPERATION_DONT_CARE = 0x0,
    RENDER_TARGET_ATTACHMENT_LOAD_OPERATION_LOAD = 0x1
} E_RENDER_TARGET_ATTACHMENT_LOAD_OPERATION;

typedef enum E_RENDER_TARGET_ATTACHMENT_STORE_OPERATION {
    RENDER_TARGET_ATTACHMENT_STORE_OPERATION_DONT_CARE = 0x0,
    RENDER_TARGET_ATTACHMENT_STORE_OPERATION_STORE = 0x1
} E_RENDER_TARGET_ATTACHMENT_STORE_OPERATION;

typedef struct RENDER_TARGET_ATTACHMENT_CONFIG {
    E_RENDER_TARGET_ATTACHMENT_TYPE type;
    E_RENDER_TARGET_ATTACHMENT_SOURCE source;
    E_RENDER_TARGET_ATTACHMENT_LOAD_OPERATION load_operation;
    E_RENDER_TARGET_ATTACHMENT_STORE_OPERATION store_operation;
    b8 present_after;
...
typedef enum E_RENDERER_CONFIG_FLAG_BITS {
    ...
    /** @brief Configures the renderer plugin in a way that conserves power where possible. */
    RENDERER_CONFIG_FLAG_POWER_SAVING_BIT = 0x2,
} E_RENDERER_CONFIG_FLAG_BITS;

typedef u32 RENDERER_CONFIG_FLAGS;

/** @brief The generic configuration for a renderer plugin. */
typedef struct RENDERER_BACKEND_CONFIG {
    /** @brief The name of the application */
    const char* application_name;
    /** @brief Various configuration flags for renderer plugin setup. */
...
typedef struct RENDERER_PLUGIN {
    ...
    b8 (*init)(struct RENDERER_PLUGIN* plugin, const RENDERER_BACKEND_CONFIG* config, u8* out_window_render_target_count);
     ...
     */
    void (*viewport_set)(struct RENDERER_PLUGIN* plugin, Vector4 rect);
     ...
     */
    void (*scissor_set)(struct RENDERER_PLUGIN* plugin, Vector4 rect);
    ...
    void (*texture_destroy)(struct RENDERER_PLUGIN* plugin, TEXTURE* texture);
     ...
     */
    b8 (*shader_create)(struct RENDERER_PLUGIN* plugin, struct SHADER* shader, const SHADER_CONFIG* config, RENDERPASS* pass, u8 stage_count, const char** stage_filenames, E_SHADER_STAGE* stages);
     ...
     */
    b8 (*shader_init)(struct RENDERER_PLUGIN* plugin, struct SHADER* shader);
    ...
    b8 (*shader_after_renderpass)(struct RENDERER_PLUGIN* plugin, struct SHADER* s);
    ...
    /**
     * @brief Creates and assigns the renderer-plugin-specific buffer.
     *
     * 
     * @param plugin A pointer to the renderer plugin interface.
     ...
     */
    b8 (*renderbuffer_create_internal)(struct RENDERER_PLUGIN* plugin, RENDER_BUFFER* buffer);
     ...
     */
    void (*renderbuffer_destroy_internal)(struct RENDERER_PLUGIN* plugin, RENDER_BUFFER* buffer);
     ...
     * @returns True on success; otherwise false.
     */
    b8 (*renderbuffer_bind)(struct RENDERER_PLUGIN* plugin, RENDER_BUFFER* buffer, u64 offset);
    /**
     ...
     * @returns True on success; otherwise false.
     */
    b8 (*renderbuffer_unbind)(struct RENDERER_PLUGIN* plugin, RENDER_BUFFER* buffer);
     ...
     */
    void* (*renderbuffer_map_memory)(struct RENDERER_PLUGIN* plugin, RENDER_BUFFER* buffer, u64 offset, u64 size);
    /**
     ...
     */
    void (*renderbuffer_unmap_memory)(struct RENDERER_PLUGIN* plugin, RENDER_BUFFER* buffer, u64 offset, u64 size);
     ...
     * @returns True on success; otherwise false.
     */
    b8 (*renderbuffer_flush)(struct RENDERER_PLUGIN* plugin, RENDER_BUFFER* buffer, u64 offset, u64 size);
     ...
     * @returns True on success; otherwise false.
     */
    b8 (*renderbuffer_read)(struct RENDERER_PLUGIN* plugin, RENDER_BUFFER* buffer, u64 offset, u64 size, void** out_memory);
     ...
     * @returns True on success; otherwise false.
     */
    b8 (*renderbuffer_resize)(struct RENDERER_PLUGIN* plugin, RENDER_BUFFER* buffer, u64 new_total_size);
     ...
     * @returns True on success; otherwise false.
     */
    b8 (*renderbuffer_load_range)(struct RENDERER_PLUGIN* plugin, RENDER_BUFFER* buffer, u64 offset, u64 size, const void* data);
     ...
     * @returns True on success; otherwise false.
     */
    b8 (*renderbuffer_copy_range)(struct RENDERER_PLUGIN* plugin, RENDER_BUFFER* source, u64 source_offset, RENDER_BUFFER* dest, u64 dest_offset, u64 size);
     ...
     * @return True on success; otherwise false.
     */
    b8 (*renderbuffer_draw)(struct RENDERER_PLUGIN* plugin, RENDER_BUFFER* buffer, u64 offset, u32 element_count, b8 bind_only);
} RENDERER_PLUGIN;
