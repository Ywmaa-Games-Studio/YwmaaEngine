...
#define BUILTIN_SHADER_NAME_MATERIAL "builtin.shader"
...
b8 vulkan_material_shader_create(VULKAN_CONTEXT* context, VULKAN_MATERIAL_SHADER* out_shader) {
    ...
    for (u32 i = 0; i < MATERIAL_SHADER_STAGE_COUNT; ++i) {
        if (!create_shader_module(context, BUILTIN_SHADER_NAME_MATERIAL, stage_type_strs[i], stage_types[i], i, out_shader->stages)) {
            PRINT_ERROR("Unable to create %s shader module for '%s'.", stage_type_strs[i], BUILTIN_SHADER_NAME_MATERIAL);
            return false;
        }
    }
    ...
    yzero_memory(&bindings, sizeof(VkDescriptorSetLayoutBinding) * VULKAN_MATERIAL_SHADER_DESCRIPTOR_COUNT);
    ...
    yzero_memory(stage_create_infos, sizeof(stage_create_infos));
    for (u32 i = 0; i < MATERIAL_SHADER_STAGE_COUNT; ++i) {
    ...
    if (!vulkan_buffer_create(
            context,
            sizeof(MATERIAL_SHADER_GLOBAL_UBO) * 3,
    ...
    if (!vulkan_buffer_create(
        context,
        sizeof(MATERIAL_SHADER_INSTANCE_UBO) * VULKAN_MAX_MATERIAL_COUNT,  //* MAX_MATERIAL_INSTANCE_COUNT,
        VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
        true,
        &out_shader->object_uniform_buffer)) {
        PRINT_ERROR("Material instance buffer creation failed for shader.");
        return false;
    }
...
void vulkan_material_shader_update_global_state(VULKAN_CONTEXT* context, struct VULKAN_MATERIAL_SHADER* shader, f32 delta_time) {
    ...
    u32 range = sizeof(MATERIAL_SHADER_GLOBAL_UBO);
    u64 offset = sizeof(MATERIAL_SHADER_GLOBAL_UBO)*image_index;
...
void vulkan_material_shader_apply_material(VULKAN_CONTEXT* context, struct VULKAN_MATERIAL_SHADER* shader, MATERIAL* material) {
    ...
    yzero_memory(descriptor_writes, sizeof(VkWriteDescriptorSet) * VULKAN_MATERIAL_SHADER_DESCRIPTOR_COUNT);
    u32 descriptor_count = 0;
    u32 descriptor_index = 0;

    // Descriptor 0 - Uniform buffer
    u32 range = sizeof(MATERIAL_SHADER_INSTANCE_UBO);
    u64 offset = sizeof(MATERIAL_SHADER_INSTANCE_UBO) * material->internal_id;  // also the index into the array.
    MATERIAL_SHADER_INSTANCE_UBO instance_obo;
...
/*         static f32 accumulator = 0.0f;
    ...
    instance_obo.diffuse_color = material->diffuse_colour;
    instance_obo.v_reserved0 = Vector4_zero();
    instance_obo.v_reserved1 = Vector4_zero();
    instance_obo.v_reserved2 = Vector4_zero();

    // Load the data into the buffer.
    vulkan_buffer_load_data(context, &shader->object_uniform_buffer, offset, range, 0, &instance_obo);
...
b8 vulkan_material_shader_acquire_resources(VULKAN_CONTEXT* context, struct VULKAN_MATERIAL_SHADER* shader, MATERIAL* material) {
    ...
    if (result != VK_SUCCESS) {
        PRINT_ERROR("Error allocating descriptor sets in shader!");
        return false;
    }
