...
b8 vulkan_ui_shader_create(VULKAN_CONTEXT* context, VULKAN_UI_SHADER* out_shader) {
    ...
    for (u32 i = 0; i < UI_SHADER_STAGE_COUNT; ++i) {
        if (!create_shader_module(context, BUILTIN_SHADER_NAME_UI, stage_type_strs[i], stage_types[i], i, out_shader->stages)) {
            PRINT_ERROR("Unable to create %s shader module for '%s'.", stage_type_strs[i], BUILTIN_SHADER_NAME_UI);
            return false;
        }
    }
    ...
    yzero_memory(&bindings, sizeof(VkDescriptorSetLayoutBinding) * VULKAN_UI_SHADER_DESCRIPTOR_COUNT);
...
#define ATTRIBUTE_COUNT 2
    ...
    if (!vulkan_buffer_create(
            context,
            sizeof(UI_SHADER_INSTANCE_UBO) * VULKAN_MAX_UI_COUNT,
            VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
            VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
            true,
            &out_shader->object_uniform_buffer)) {
        PRINT_ERROR("UI instance buffer creation failed for shader.");
        return false;
    }
...
void vulkan_ui_shader_update_global_state(VULKAN_CONTEXT* context, struct VULKAN_UI_SHADER* shader, f32 delta_time) {
    ...
    u32 range = sizeof(UI_SHADER_GLOBAL_UBO);
    u64 offset = 0;
...
void vulkan_ui_shader_apply_material(VULKAN_CONTEXT* context, struct VULKAN_UI_SHADER* shader, MATERIAL* material) {
    if (context && shader) {
        ...
        yzero_memory(descriptor_writes, sizeof(VkWriteDescriptorSet) * VULKAN_UI_SHADER_DESCRIPTOR_COUNT);
        ...
        UI_SHADER_INSTANCE_UBO instance_ubo;
