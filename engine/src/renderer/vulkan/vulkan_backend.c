...
b8 vulkan_renderer_shader_create(SHADER* shader, RENDERPASS* pass, u8 stage_count, const char** stage_filenames, E_SHADER_STAGE* stages) {
    shader->internal_data = yallocate_aligned(sizeof(VULKAN_SHADER), 8, MEMORY_TAG_RENDERER);

    PRINT_DEBUG("vulkan_renderer_shader_create: Creating shader with %d stages.", stage_count);
    // Translate stages
    ...
    VULKAN_DESCRIPTOR_SET_CONFIG global_descriptor_set_config = {0};
    ...
    if (shader->use_instances) {
        ...
        VULKAN_DESCRIPTOR_SET_CONFIG instance_descriptor_set_config = {0};
