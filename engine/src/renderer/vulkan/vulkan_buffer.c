...
b8 vulkan_buffer_create(
    ...
    yallocate_report(out_buffer->memory_requirements.size, is_device_memory ? MEMORY_TAG_GPU_LOCAL : MEMORY_TAG_VULKAN);
...
void vulkan_buffer_destroy(VULKAN_CONTEXT* context, VULKAN_BUFFER* buffer) {
    ...
    yfree_report(buffer->memory_requirements.size, is_device_memory ? MEMORY_TAG_GPU_LOCAL : MEMORY_TAG_VULKAN);
    yzero_memory(&buffer->memory_requirements, sizeof(VkMemoryRequirements));
...
b8 vulkan_buffer_resize(
    ...
    yallocate_report(buffer->memory_requirements.size, is_device_memory ? MEMORY_TAG_GPU_LOCAL : MEMORY_TAG_VULKAN);
