...
void vulkan_device_destroy(VULKAN_CONTEXT* context) {
    destroy_logical_device(context);
}
...
b8 select_physical_device(VULKAN_CONTEXT* context) {
    ...
    for (u32 i = 0; i < physical_device_count; ++i) {
        ...
        if (result) {
            PRINT_INFO("Selected device: '%s'.", properties.deviceName);
            // GPU type, etc.
            switch (properties.deviceType) {
                default:
                case VK_PHYSICAL_DEVICE_TYPE_OTHER:
                    PRINT_INFO("GPU type is Unknown.");
                    break;
                case VK_PHYSICAL_DEVICE_TYPE_INTEGRATED_GPU:
                    PRINT_INFO("GPU type is Integrated.");
                    break;
                case VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU:
                    PRINT_INFO("GPU type is Descrete.");
                    break;
                case VK_PHYSICAL_DEVICE_TYPE_VIRTUAL_GPU:
                    PRINT_INFO("GPU type is Virtual.");
                    break;
                case VK_PHYSICAL_DEVICE_TYPE_CPU:
                    PRINT_INFO("GPU type is CPU.");
                    break;
            }
            ...
            for (u32 j = 0; j < memory.memoryHeapCount; ++j) {
                ...
                if (memory.memoryHeaps[j].flags & VK_MEMORY_HEAP_DEVICE_LOCAL_BIT) {
                    PRINT_INFO("Local GPU memory: %.2f GiB", memory_size_gib);
                } else {
                    PRINT_INFO("Shared System memory: %.2f GiB", memory_size_gib);
                }
            }
    ...
    if (!context->device.physical_device) {
        PRINT_ERROR("No physical devices were found which meet the requirements.");
        return false;
    }

    PRINT_INFO("Physical device selected.");
    return true;
}
...
b8 physical_device_meets_requirements(
    ...
    if (requirements->discrete_gpu) {
        if (properties->deviceType != VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU) {
            PRINT_INFO("Device is not a discrete GPU, and one is required. Skipping.");
            return false;
        }
    }
    ...
    PRINT_INFO("Graphics | Present | Compute | Transfer | Name");
    ...
    if (
        ...
        PRINT_INFO("Device meets queue requirements.");
        PRINT_INFO("Graphics Family Index: %i", out_queue_info->graphics_family_index);
        PRINT_INFO("Present Family Index:  %i", out_queue_info->present_family_index);
        PRINT_INFO("Transfer Family Index: %i", out_queue_info->transfer_family_index);
        PRINT_INFO("Compute Family Index:  %i", out_queue_info->compute_family_index);
        ...
        if (out_swapchain_support->format_count < 1 || out_swapchain_support->present_mode_count < 1) {
            if (out_swapchain_support->formats) {
                yfree_aligned(out_swapchain_support->formats, sizeof(VkSurfaceFormatKHR) * out_swapchain_support->format_count, 4, MEMORY_TAG_RENDERER);
            }
            if (out_swapchain_support->present_modes) {
                yfree_aligned(out_swapchain_support->present_modes, sizeof(VkPresentModeKHR) * out_swapchain_support->present_mode_count, 4, MEMORY_TAG_RENDERER);
            }
            ...
            if (available_extension_count != 0) {
                ...
                for (u32 i = 0; i < required_extension_count; ++i) {
                    ...
                    if (!found) {
                        ...
                        yfree_aligned(available_extensions, sizeof(VkExtensionProperties) * available_extension_count, 4, MEMORY_TAG_RENDERER);
                        return false;
                    }
                }
            }
        ...
        if (requirements->sampler_anisotropy && !features->samplerAnisotropy) {
            PRINT_INFO("Device does not support samplerAnisotropy, skipping.");
            return false;
        }
...
void create_logical_device(VULKAN_CONTEXT* context){
    PRINT_INFO("Creating logical device...");
    ...
    if (available_extension_count != 0) {
        ...
        for (u32 i = 0; i < available_extension_count; ++i) {
            if (strings_equal(available_extensions[i].extensionName, "VK_KHR_portability_subset")) {
                PRINT_INFO("Adding required extension 'VK_KHR_portability_subset'.");
    ...
    PRINT_INFO("Logical device created.");
...
void destroy_logical_device(VULKAN_CONTEXT* context){
    ...
    PRINT_INFO("Releasing physical device resources...");
