...
void vulkan_swapchain_present(
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR) {
        ...
        PRINT_ERROR("Failed to present swap chain image!");
    }
...
void create(VULKAN_CONTEXT* context, u32 width, u32 height, VULKAN_SWAPCHAIN* swapchain) {
    ...
    if (!swapchain->render_textures) {
        swapchain->render_textures = (TEXTURE*)yallocate_aligned(sizeof(TEXTURE) * swapchain->image_count, 8, MEMORY_TAG_RENDERER);
        // If creating the array, then the internal texture objects aren't created yet either.
        for (u32 i = 0; i < swapchain->image_count; ++i) {
            void* internal_data = yallocate_aligned(sizeof(VULKAN_IMAGE), 8, MEMORY_TAG_TEXTURE);
    ...
    if (!swapchain->depth_textures) {
        swapchain->depth_textures = (TEXTURE*)yallocate_aligned(sizeof(TEXTURE) * swapchain->image_count, 8, MEMORY_TAG_RENDERER);
    }
