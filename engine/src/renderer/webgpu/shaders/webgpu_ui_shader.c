...
#define BUILTIN_SHADER_NAME_OBJECT "ui_shader"
#define BINDINGS_ENTRY_COUNT 3
#define TEXTURES_BINDINGS_ENTRY_COUNT 2

#include "math/ymath.h"

void webgpu_ui_create_bind_group(WEBGPU_CONTEXT* context, WEBGPU_UI_SHADER* shader);
void webgpu_ui_create_textures_bind_group(WEBGPU_CONTEXT* context, WGPUTextureView* view, WGPUSampler* sampler, WEBGPU_UI_SHADER* shader, WEBGPU_UI_SHADER_INSTANCE_STATE* object_state);
void webgpu_ui_bind_layout_set_default(WGPUBindGroupLayoutEntry *bindingLayout);

b8 webgpu_ui_shader_create(WEBGPU_CONTEXT* context, WEBGPU_UI_SHADER* out_shader) {


    if (!webgpu_create_shader_module(context, &out_shader->shader_module, BUILTIN_SHADER_NAME_OBJECT)) {
    ...
    webgpu_ui_bind_layout_set_default(&binding_layout[2]);
    binding_layout[2].nextInChain = NULL;
    binding_layout[2].binding = 2;
    binding_layout[2].buffer.type = WGPUBufferBindingType_Uniform;
    binding_layout[2].buffer.minBindingSize = sizeof(UI_SHADER_INSTANCE_UBO);
    ...
    webgpu_ui_bind_layout_set_default(&texture_binding_layout[0]);
    ...
    webgpu_ui_bind_layout_set_default(&texture_binding_layout[1]);
    ...
    if (!webgpu_pipeline_create(context, vertex_buffer_layout, &out_shader->pipeline, &out_shader->shader_module)) {
    ...
    if (!webgpu_buffer_create(
        context,
        sizeof(UI_SHADER_INSTANCE_UBO) * WEBGPU_MAX_UI_SHADER_COUNT,
        WGPUBufferUsage_CopyDst | WGPUBufferUsage_Uniform,
        false,
    ...
    webgpu_ui_create_bind_group(context, out_shader);
...
void webgpu_ui_shader_use(WEBGPU_CONTEXT* context, struct WEBGPU_UI_SHADER* shader) {
    ...
    wgpuRenderPassEncoderSetPipeline(context->ui_render_pass, shader->pipeline.handle);
    
    // Set binding group here!
    u32 dynamic_offsets[3] = {0, 0, sizeof(UI_SHADER_INSTANCE_UBO)};
    wgpuRenderPassEncoderSetBindGroup(context->ui_render_pass, 0, shader->bind_group, 1, dynamic_offsets);
}

void webgpu_ui_shader_update_global_state(WEBGPU_CONTEXT* context, struct WEBGPU_UI_SHADER* shader, f32 delta_time) {
    if (!context || !shader) {
        PRINT_ERROR("webgpu_ui_shader_update_global_state: context or shader is null.");
        return;
    }
    // Update uniform buffer
    wgpuQueueWriteBuffer(context->queue, shader->global_uniform_buffer.handle, 0, &shader->global_ubo, sizeof(UI_SHADER_GLOBAL_UBO));
}

void webgpu_ui_shader_set_model(WEBGPU_CONTEXT* context, struct WEBGPU_UI_SHADER* shader, Matrice4 model) {
    if (!context || !shader) {
        PRINT_ERROR("webgpu_ui_shader_set_model: context or shader is null.");
        return;
    }
...
void webgpu_ui_shader_apply_material(WEBGPU_CONTEXT* context, struct WEBGPU_UI_SHADER* shader, MATERIAL* material) {
    if (!context || !shader) {
        PRINT_ERROR("webgpu_ui_shader_apply_material: context or shader is null.");
        return;
    }
    // Obtain material data.
    WEBGPU_UI_SHADER_INSTANCE_STATE* object_state = &shader->instance_states[material->internal_id];

    //u32 range = sizeof(UI_SHADER_INSTANCE_UBO);
    u64 offset = sizeof(UI_SHADER_INSTANCE_UBO) * material->internal_id;  // also the index into the array.
    UI_SHADER_INSTANCE_UBO obo;
    ...
    wgpuQueueWriteBuffer(context->queue, shader->object_uniform_buffer.handle, offset, &obo, sizeof(UI_SHADER_INSTANCE_UBO));
    ...
    for (u32 sampler_index = 0; sampler_index < sampler_count; ++sampler_index) {
        ...
        if (t && (*descriptor_id != t->id || *descriptor_generation != t->generation || *descriptor_generation == INVALID_ID)) {
            ...
            webgpu_ui_create_textures_bind_group(context, &internal_data->image.view, &internal_data->sampler, shader, object_state);
    ...
    wgpuRenderPassEncoderSetBindGroup(context->ui_render_pass, 1, object_state->texture_bind_group, 0, NULL);
}
...
void webgpu_ui_shader_release_resources(WEBGPU_CONTEXT* context, struct WEBGPU_UI_SHADER* shader, MATERIAL* material) {
    WEBGPU_UI_SHADER_INSTANCE_STATE* instance_state = &shader->instance_states[material->internal_id];
...
void webgpu_ui_create_bind_group(WEBGPU_CONTEXT* context, WEBGPU_UI_SHADER* shader){
    ...
    binding[0].size = sizeof(UI_SHADER_GLOBAL_UBO);
    ...
    binding[2].offset = 0;
    // And we specify again the size of the buffer.
    binding[2].size = sizeof(UI_SHADER_INSTANCE_UBO) * WEBGPU_MAX_UI_SHADER_COUNT;
