...
b8 shader_loader_load(struct RESOURCE_LOADER* self, const char* name, RESOURCE* out_resource) {
    ...
    if (!filesystem_open(full_file_path, FILE_MODE_READ, false, &f)) {
        PRINT_ERROR("shader_loader_load - unable to open shader file for reading: '%s'.", full_file_path);
        return false;
    }
    ...
    while (filesystem_read_line(&f, 511, &p, &line_length)) {
        ...
        if (strings_equali(trimmed_var_name, "version")) {
            ...
            for (u8 i = 0; i < resource_data->stage_count; ++i) {
                if (strings_equali(stage_names[i], "fragment") || strings_equali(stage_names[i], "fragment")) {
                    darray_push(resource_data->stages, SHADER_STAGE_FRAGMENT);
                } else if (strings_equali(stage_names[i], "vertex") || strings_equali(stage_names[i], "vertex")) {
                    darray_push(resource_data->stages, SHADER_STAGE_VERTEX);
                } else if (strings_equali(stage_names[i], "geometry") || strings_equali(stage_names[i], "geometry")) {
                    darray_push(resource_data->stages, SHADER_STAGE_GEOMETRY);
                } else if (strings_equali(stage_names[i], "compute") || strings_equali(stage_names[i], "compute")) {
                    darray_push(resource_data->stages, SHADER_STAGE_COMPUTE);
                } else {
                    PRINT_ERROR("shader_loader_load: Invalid file layout. Unrecognized stage '%s'", stage_names[i]);
                }
            }
        } else if (strings_equali(trimmed_var_name, "vulkan_stagefiles")) {
            // Parse the stage file names
            resource_data->vulkan_stage_filenames = darray_create(char*);
            u32 count = string_split(trimmed_value, ',', &resource_data->vulkan_stage_filenames, true, true);
            // Ensure stage name and stage file name count are the same, as they should align.
            if (resource_data->stage_count == 0) {
                resource_data->stage_count = count;
            } else if (resource_data->stage_count != count) {
        ...
        } else if (strings_equali(trimmed_var_name, "webgpu_stagefiles")) {
            // Parse the stage file names
            resource_data->webgpu_stage_filenames = darray_create(char*);
            u32 count = string_split(trimmed_value, ',', &resource_data->webgpu_stage_filenames, true, true);
            // Ensure stage name and stage file name count are the same, as they should align.
            if (resource_data->stage_count == 0) {
                resource_data->stage_count = count;
            } else if (resource_data->stage_count != count) {
            ...
            if (field_count != 2) {
                PRINT_ERROR("shader_loader_load: Invalid file layout. Attribute fields must be 'type,name'. Skipping.");
            } else {
                ...
                if (strings_equali(fields[0], "f32")) {
                    ...
                    PRINT_ERROR("shader_loader_load: Invalid file layout. Attribute type must be f32, vec2, vec3, vec4, i8, i16, i32, u8, u16, or u32.");
            ...
            u32 field_count = string_split(trimmed_value, ',', &fields, true, true);
            if (field_count != 3) {
                PRINT_ERROR("shader_loader_load: Invalid file layout. Uniform fields must be 'type,scope,name'. Skipping.");
            } else {
                ...
                if (strings_equal(fields[1], "0")) {
                    ...
                    PRINT_ERROR("shader_loader_load: Invalid file layout: Uniform scope must be 0 for global, 1 for instance or 2 for local.");
                    PRINT_WARNING("Defaulting to global.");
                    uniform.scope = SHADER_SCOPE_GLOBAL;
                }
...
void shader_loader_unload(struct RESOURCE_LOADER* self, RESOURCE* resource) {
    ...
    string_cleanup_split_array(data->webgpu_stage_filenames);
    darray_destroy(data->vulkan_stage_filenames);
    darray_destroy(data->webgpu_stage_filenames);
    ...
    if (!resource_unload(self, resource, MEMORY_TAG_RESOURCE)) {
        PRINT_WARNING("shader_loader_unload called with nullptr for self or resource.");
    }
}
