...
b8 skybox_create(SKYBOX_CONFIG config, Skybox* out_skybox) {
    if (!out_skybox) {
        PRINT_ERROR("skybox_create requires a valid pointer to out_skybox!");
        return false;
    }
...
b8 skybox_init(Skybox* sb) {
    if (!sb) {
        PRINT_ERROR("skybox_init requires a valid pointer to sb!");
        return false;
    }
...
b8 skybox_load(Skybox* sb) {
    if (!sb) {
        PRINT_ERROR("skybox_load requires a valid pointer to sb!");
        return false;
    }
...
b8 skybox_unload(Skybox* sb) {
    if (!sb) {
        PRINT_ERROR("skybox_unload requires a valid pointer to sb!");
        return false;
    }
    ...
    if (sb->config.cubemap_name) {
        ...
        // yfree((void*)sb->config.cubemap_name);
...
void skybox_destroy(Skybox* sb) {
    if (!sb) {
        PRINT_ERROR("skybox_destroy requires a valid pointer to sb!");
        return;
    }
    ...
    if (sb->instance_id != INVALID_ID) {
        ...
        if (!result) {
            PRINT_ERROR("skybox_destroy() - Failed to successfully unload skybox before destruction.");
        }
    }

    yzero_memory(sb, sizeof(Skybox));
}
