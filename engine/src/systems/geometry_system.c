...
b8 geometry_system_init(u64* memory_requirement, void* state, GEOMETRY_SYSTEM_CONFIG config) {
    if (config.max_geometry_count == 0) {
        PRINT_ERROR("geometry_system_initialize - config.max_geometry_count must be > 0.");
        return false;
    }
...
GEOMETRY_CONFIG geometry_system_generate_cube_config(f32 width, f32 height, f32 depth, f32 tile_x, f32 tile_y, const char* name, const char* material_name) {
    ...
    verts[(0 * 4) + 0].normal = (Vector3){0, 0, -1};
    verts[(0 * 4) + 1].normal = (Vector3){0, 0, -1};
    verts[(0 * 4) + 2].normal = (Vector3){0, 0, -1};
    verts[(0 * 4) + 3].normal = (Vector3){0, 0, -1};
    ...
    verts[(1 * 4) + 0].normal = (Vector3){0, 0, 1};
    verts[(1 * 4) + 1].normal = (Vector3){0, 0, 1};
    verts[(1 * 4) + 2].normal = (Vector3){0, 0, 1};
    verts[(1 * 4) + 3].normal = (Vector3){0, 0, 1};
