...
b8 resource_system_init(u64* memory_requirement, void* state, void* config) {
    ...
    if (typed_config->max_loader_count == 0) {
        PRINT_ERROR("resource_system_init failed because typed_config->max_loader_count==0.");
        return false;
    }
    ...
    *memory_requirement = sizeof(RESOURCE_SYSTEM_STATE) + (sizeof(RESOURCE_LOADER) * typed_config->max_loader_count);
    ...
    PRINT_INFO("Resource system initialized with base path '%s'.", typed_config->asset_base_path);
        ...
        for (u32 i = 0; i < count; ++i) {
            ...
            if (l->id != INVALID_ID) {
                if (l->type == loader.type) {
                    PRINT_ERROR("resource_system_loader_register - Loader of type %d already exists and will not be registered.", loader.type);
                    return false;
                } else if (loader.custom_type && string_length(loader.custom_type) > 0 && strings_equali(l->custom_type, loader.custom_type)) {
                    PRINT_ERROR("resource_system_loader_register - Loader of custom type %s already exists and will not be registered.", loader.custom_type);
                    return false;
                }
            }
        }
...
b8 resource_system_load(const char* name, E_RESOURCE_TYPE type, void* params, RESOURCE* out_resource) {
    ...
    PRINT_ERROR("resource_system_load - No loader for type %d was found.", type);
    return false;
}
...
b8 resource_system_load_custom(const char* name, const char* custom_type, void* params, RESOURCE* out_resource) {
    ...
    out_resource->loader_id = INVALID_ID;
    PRINT_ERROR("resource_system_load_custom - No loader for type %s was found.", custom_type);
    return false;
}
...
const char* resource_system_base_path(void) {
    ...
    PRINT_ERROR("resource_system_base_path called before initialization, returning empty string.");
    return "";
}
