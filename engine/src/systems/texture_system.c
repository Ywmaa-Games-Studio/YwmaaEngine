...
TEXTURE* texture_system_acquire(const char* name, b8 auto_release) {
    ...
    if (state_ptr && hashtable_get(&state_ptr->registered_texture_table, name, &ref)) {
        ...
        if (ref.handle == INVALID_ID) {
            ...
            if (!t || ref.handle == INVALID_ID) {
                PRINT_ERROR("texture_system_acquire - Texture system cannot hold anymore textures. Adjust configuration to allow more.");
                return 0;
            }
            ...
            t->id = ref.handle;
            PRINT_DEBUG("Texture '%s' does not yet exist. Created, and ref_count is now %i.", name, ref.reference_count);
        } else {
            PRINT_DEBUG("Texture '%s' already exists, ref_count increased to %i.", name, ref.reference_count);
        }
    ...
    PRINT_ERROR("texture_system_acquire failed to acquire texture '%s'. Null pointer will be returned.", name);
    return 0;
}
...
void texture_system_release(const char* name) {
    ...
    if (state_ptr && hashtable_get(&state_ptr->registered_texture_table, name, &ref)) {
        ...
        if (ref.reference_count == 0 && ref.auto_release) {
            ...
            PRINT_DEBUG("Released texture '%s'., Texture unloaded because reference count=0 and auto_release=true.", name_copy);
        } else {
            PRINT_DEBUG("Released texture '%s', now has a reference count of '%i' (auto_release=%s).", name_copy, ref.reference_count, ref.auto_release ? "true" : "false");
        }

        // Update the entry.
        hashtable_set(&state_ptr->registered_texture_table, name_copy, &ref);
    } else {
        PRINT_ERROR("texture_system_release failed to release texture '%s'.", name);
    }
}
...
TEXTURE* texture_system_get_default_texture() {
    ...
    PRINT_ERROR("texture_system_get_default_texture called before texture system initialization! Null pointer returned.");
    return 0;
}
...
b8 load_texture(const char* texture_name, TEXTURE* t) {
    ...
    if (data) {
        ...
        if (stbi_failure_reason()) {
            PRINT_WARNING("load_texture() failed to load file '%s': %s", full_file_path, stbi_failure_reason());
