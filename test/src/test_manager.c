...
void test_manager_run_tests() {
    ...
    for (u32 i = 0; i < count; ++i) {
        ...
        if (result == true) {
            ...
            PRINT_WARNING("[SKIPPED]: %s", tests[i].desc);
            ++skipped;
        } else {
            PRINT_ERROR("[FAILED]: %s", tests[i].desc);
            ++failed;
        }
        char status[20];
        string_format(status, failed ? "*** %d FAILED ***" : "SUCCESS", failed);
        clock_update(&total_time);
        PRINT_INFO("Executed %d of %d (skipped %d) %s (%.6f sec / %.6f sec total", i + 1, count, skipped, status, test_time.elapsed, total_time.elapsed);
    }

    clock_stop(&total_time);

    PRINT_INFO("Results: %d passed, %d failed, %d skipped.", passed, failed, skipped);
}