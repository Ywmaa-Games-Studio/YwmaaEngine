            ...
            for (u32 i = 0; i < queue_family_count; ++i) {
                ...
                if (supports_present) {
                    ...
                    if (out_queue_info->present_family_index != out_queue_info->graphics_family_index) {
                        PRINT_WARNING("Warning: Different queue index used for present vs graphics: %u.", i);
                    }
                    break;
                }
            }
    ...
    if (
        ...
        if (out_swapchain_support->format_count < 1 || out_swapchain_support->present_mode_count < 1) {
            if (out_swapchain_support->formats) {
                yfree(out_swapchain_support->formats);
            }
            if (out_swapchain_support->present_modes) {
                yfree(out_swapchain_support->present_modes);
            }
