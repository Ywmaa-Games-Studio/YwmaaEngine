...
void vulkan_image_create(
    ...
    if (memory_type == -1) {
        PRINT_ERROR("Required memory type not found. Image not valid.");
    }
    ...
    yallocate_report(out_image->memory_requirements.size, is_device_memory ? MEMORY_TAG_GPU_LOCAL : MEMORY_TAG_VULKAN);
...
void vulkan_image_copy_from_buffer(
    ...
    yzero_memory(&region, sizeof(VkBufferImageCopy));
...
void vulkan_image_destroy(VULKAN_CONTEXT* context, VULKAN_IMAGE* image) {
    ...
    yfree_report(image->memory_requirements.size, is_device_memory ? MEMORY_TAG_GPU_LOCAL : MEMORY_TAG_VULKAN);
    yzero_memory(&image->memory_requirements, sizeof(VkMemoryRequirements));
}
