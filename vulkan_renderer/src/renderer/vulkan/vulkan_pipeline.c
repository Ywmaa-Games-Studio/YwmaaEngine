...
b8 vulkan_graphics_pipeline_create(VULKAN_CONTEXT* context, const VULKAN_PIPELINE_CONFIG* config, VULKAN_PIPELINE* out_pipeline) {
    ...
    if (config->push_constant_range_count > 0) {
        if (config->push_constant_range_count > 32) {
            PRINT_ERROR("vulkan_graphics_pipeline_create: cannot have more than 32 push constant ranges. Passed count: %i", config->push_constant_range_count);
            return false;
        }

        // NOTE: 32 is the max number of ranges we can ever have, since spec only guarantees 128 bytes with 4-byte alignment.
        yzero_memory(ranges, sizeof(VkPushConstantRange) * 32);
    ...
    if (vulkan_result_is_success(result)) {
        PRINT_DEBUG("Graphics pipeline created!");
        return true;
    }

    PRINT_ERROR("vkCreateGraphicsPipelines failed with %s.", vulkan_result_string(result, true));
    return false;
}
